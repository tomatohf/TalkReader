package com.qiaobutang.talk.reader.control {
	...
	public class ScrollingText extends UIComponent
	{
		...
		override protected function createChildren():void
		{
			...
			// added by Tomato
			(cache[0] as Label).selectable = true;
			(cache[1] as Label).selectable = true;
			var label_link_handler:Function = function(event:TextEvent):void {
				navigateToURL(new URLRequest(event.text), "_blank");
			}
			Label(cache[0]).addEventListener(TextEvent.LINK, label_link_handler);
			Label(cache[1]).addEventListener(TextEvent.LINK, label_link_handler);
			
			this.addEventListener(MouseEvent.MOUSE_OVER, mouse_event_handler);
			this.addEventListener(MouseEvent.MOUSE_OUT, mouse_event_handler);
		}
		
		// Added by Tomato
		private function mouse_event_handler(event:MouseEvent):void {
