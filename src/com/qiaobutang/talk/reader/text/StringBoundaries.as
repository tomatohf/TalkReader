	...
	public class StringBoundaries
	{
		...
		*/
		public function StringBoundaries(textField:TextField,startIndex:int,endIndex:int,xOffset:Number=0,yOffset:Number=0)
		{
			...
			/*------------------------------------------------//
			Little modification of the endIndex as the highlight
			was breaking if the endIndex was the EOL.
			
			@author S S Virk	(virkster@gmail.com)
			//------------------------------------------------*/
			if(endIndex == textField.getLineOffset(textField.getLineIndexOfChar(endIndex)) + textField.getLineLength(textField.getLineIndexOfChar(endIndex)) - 1){
		...
		*/
		private function stringRect(startIndex:int, endIndex:int):Rectangle{
			...
			//changed rect2.right to rect2.x
		...
		*/
		private function getLineEndOffset(lineIndex:int):int{
			//changed the textField.getLineLength(lineIndex) to (textField.getLineLength(lineIndex) - 2)
			//makes it possible for the Highlighter to span multiple lines. -- SS Virk
			return textField.getLineOffset(lineIndex) + (textField.getLineLength(lineIndex) - 2);
		...
		*/
		private function getAdjustedCharBoundaries(index:int, applyScrollVOffset:Boolean):Rectangle{
			...
			if(rect == null){
				...
				//I took this out for my current project, but feel free to play with it. --SS Virk
				rect.x += this.xOffset; //+this.X_CORRECTION 
